...
### Venue: NIPS 2022
本文在PAC（Probably Approximately Correct）框架下证明了，经典理论中研究（非鲁棒）多分类问题下函数类可学习性的重要工具——Natarajan维度或graph维度——无法刻画函数类的鲁棒可学习性。对此，我们定义了AN维度和AG维度（Adversarial Natarajan Dimension & Adversarial Graph Dimension），基于此导出了学习鲁棒分类器所需样本复杂度的一个上界与下界。此外作者研究了这两个定义的诸多性质。本文为填补鲁棒学习领域多分类理论的空白做出了重要贡献。
